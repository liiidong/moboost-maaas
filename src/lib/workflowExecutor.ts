...
registerNodeExecutor('batch_split', async (node, inputs) => {
  // V3 增强：适配计划模式（图片广告工作流用）
  const requirements = inputs['requirements'] as any;
  if (requirements?.rows) {
    // 从需求表生成适配计划
    const adaptationPlan = requirements.rows.map((row: any) => ({
      rowId: row.rowId,
    ...
    return {
      items: adaptationPlan,
      count: adaptationPlan.length,
      isAdaptationPlan: true,
    };
  }

  // 原有逻辑：将上游的 storyboard/prompts 按 scenes 拆分
