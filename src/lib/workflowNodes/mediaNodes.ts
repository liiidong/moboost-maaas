...
registerNodeExecutor('image_gen', async (node, inputs, config) => {
  ...
  const variantCount = imgConfig?.variantCount ?? 1;
  const negativeConstraints = imgConfig?.negativeConstraints ?? [];
  const logoReserveArea = imgConfig?.logoReserveArea;

  // 构建增强提示词（广告工作流增强）
  let enhancedPrompt = prompt;
  if (negativeConstraints.length > 0) {
    enhancedPrompt += `\n\nIMPORTANT: Do NOT include: ${negativeConstraints.join(', ')}.`;
  }
  if (logoReserveArea) {
    enhancedPrompt += `\n\nReserve a clean area at the ${logoReserveArea.position} corner (approximately ${Math.round(logoReserveArea.widthRatio * 100)}% width) for logo placement later. Keep this area clean with simple background, no text or important elements.`;
  }
  ...
  if (variantCount > 1) {
    ...
    for (let i = 0; i < variantCount; i++) {
      const variantPrompt = `${enhancedPrompt}\n\nThis is layout variant ${i + 1} of ${variantCount}. Use the same copy and campaign promise but a meaningfully different composition/layout.`;
      const result = await generateSingleImage(variantPrompt, imgConfig);
      variants.push({ ...result, variantIndex: i });
    }
    return { images: variants, variantCount, prompt };
  }

  // 单图生成（原有逻辑）
  const result = await generateSingleImage(enhancedPrompt, imgConfig);
  return { image: result.image, model: result.model, prompt };
});

/** 单张图片生成（从 image_gen 抽取） */
async function generateSingleImage(prompt: string, imgConfig: any) {
  // 使用已验证可用的模型
