...
registerNodeExecutor('human_review', async (node, inputs, config) => {
  ...
  const reviewConfig = config.humanReview;
  ...
  // V3: 广告工作流增强 — 审批类型和回退目标
  (error as any).reviewType = reviewConfig?.reviewType || 'generic';
  (error as any).rejectTarget = reviewConfig?.rejectTarget || null;
  (error as any).partialRedo = reviewConfig?.partialRedo || false;
  throw error;
});
